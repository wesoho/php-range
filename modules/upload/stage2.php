...
if ($passed) pass_stage('upload', 2, 'upload ' . ($_FILES['file']['name'] ?? ''));

upload_head(2, '前端JS验证', '仅前端 JS 检查扩展名，禁用 JS 或改请求即可绕过。');
?>
<script>
function checkExt(){var f=document.forms[0].file.value;if(!/\.(jpg|png|gif)$/.test(f)){alert("仅允许图片");return false;}return true;}
</script>
<form method="post" enctype="multipart/form-data" class="lab" onsubmit="return checkExt()">
...
<?php
upload_tail(
    ['hint' => '前端 JS 可绕过：禁用 JS 或用 curl/Burp 直接 POST', 'full' => 'curl -F file=@test.php 上传'],
    [
    '原理' => '文件上传第 2 关：仅前端 JS 检查扩展名，禁用 JS 或改请求即可绕过。<br>文件上传漏洞核心：服务器未正确验证上传文件类型，导致攻击者上传可执行文件（如 .php）获取 webshell。',
...
if ($allow) move_uploaded_file($_FILES["file"]["tmp_name"], UPLOAD_DIR.$name);</pre><span class="danger">↑ 验证不充分</span>',
    '攻击演示' => '1. 尝试上传 .php 文件<br>2. 根据验证逻辑构造绕过 payload<br>3. 上传成功后访问 /uploads/shell.php → 通关<br>payload：curl -F file=@test.php 上传',
    'payload详解' => '<code class="payload">curl -F file=@test.php 上传</code><br>提示：前端 JS 可绕过：禁用 JS 或用 curl/Burp 直接 POST',
    '工具实操' => 'curl -F file=@shell.php http://靶场/stage2.php<br>Burp Suite 抓包修改 filename / Content-Type<br>上传后访问 /uploads/验证执行',
...
$newname = bin2hex(random_bytes(16)) . "." . $ext;</pre>',
    '知识卡片' => '📌 文件上传漏洞可导致 RCE（远程代码执行）<br>OWASP A04:2021-Insecure Design<br>关键防御：白名单 > 黑名单，内容检测 > 扩展名检测<br>本关演示：前端JS验证'
],
    __FILE__
);
