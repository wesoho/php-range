...
if ($passed) pass_stage('upload', 4, 'upload ' . ($_FILES['file']['name'] ?? ''));

upload_head(4, '扩展名黑名单', '黑名单禁止 .php，但 .phtml .pht 等不在列表中。');
?>
...
<?php
upload_tail(
    ['hint' => '黑名单不含 .phtml，上传 file.phtml', 'full' => '上传 shell.phtml'],
    [
    '原理' => '文件上传第 4 关：黑名单禁止 .php，但 .phtml .pht 等不在列表中。<br>文件上传漏洞核心：服务器未正确验证上传文件类型，导致攻击者上传可执行文件（如 .php）获取 webshell。',
...
if ($allow) move_uploaded_file($_FILES["file"]["tmp_name"], UPLOAD_DIR.$name);</pre><span class="danger">↑ 验证不充分</span>',
    ...
    'payload详解' => '<code class="payload">上传 shell.phtml</code><br>提示：黑名单不含 .phtml，上传 file.phtml',
    '工具实操' => 'curl -F file=@shell.php http://靶场/stage4.php<br>Burp Suite 抓包修改 filename / Content-Type<br>上传后访问 /uploads/验证执行',
...
$newname = bin2hex(random_bytes(16)) . "." . $ext;</pre>',
    '知识卡片' => '📌 文件上传漏洞可导致 RCE（远程代码执行）<br>OWASP A04:2021-Insecure Design<br>关键防御：白名单 > 黑名单，内容检测 > 扩展名检测<br>本关演示：扩展名黑名单'
],
    __FILE__
);
