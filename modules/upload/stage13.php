...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file'])) {
    ...
    if ($level === 'impossible') {
        ...
        // low: 无过滤任意上传（条件竞争：上传后短暂存在，并发访问执行）
...
if ($passed) pass_stage('upload', 13, 'upload ' . ($_FILES['file']['name'] ?? ''));

upload_head(13, '条件竞争', '上传后删除前的时间窗口，并发访问执行。');
?>
...
<?php
upload_tail(
    ['hint' => '并发上传+访问，利用删除前窗口', 'full' => '多线程上传同时访问'],
    [
    '原理' => '文件上传第 13 关：上传后删除前的时间窗口，并发访问执行。<br>文件上传漏洞核心：服务器未正确验证上传文件类型，导致攻击者上传可执行文件（如 .php）获取 webshell。',
    '漏洞代码' => '<pre>$name = $_FILES["file"]["name"];
        $ext = pathinfo($name, PATHINFO_EXTENSION);
        $allow = in_array($ext, ["jpg","png","gif"]);
if ($allow) move_uploaded_file($_FILES["file"]["tmp_name"], UPLOAD_DIR.$name);</pre><span class="danger">↑ 验证不充分</span>',
    '攻击演示' => '1. 尝试上传 .php 文件<br>2. 根据验证逻辑构造绕过 payload<br>3. 上传成功后访问 /uploads/shell.php → 通关<br>payload：多线程上传同时访问',
    'payload详解' => '<code class="payload">多线程上传同时访问</code><br>提示：并发上传+访问，利用删除前窗口',
    '工具实操' => 'curl -F file=@shell.php http://靶场/stage13.php<br>Burp Suite 抓包修改 filename / Content-Type<br>上传后访问 /uploads/验证执行',
...
$newname = bin2hex(random_bytes(16)) . "." . $ext;</pre>',
    '知识卡片' => '📌 文件上传漏洞可导致 RCE（远程代码执行）<br>OWASP A04:2021-Insecure Design<br>关键防御：白名单 > 黑名单，内容检测 > 扩展名检测<br>本关演示：条件竞争'
],
    __FILE__
);
