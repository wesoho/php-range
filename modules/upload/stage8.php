...
if ($passed) pass_stage('upload', 8, 'upload ' . ($_FILES['file']['name'] ?? ''));

upload_head(8, '空字节绕过', '利用空字节截断，file.php%00.jpg（PHP 5.x 漏洞演示）。');
?>
...
<?php
upload_tail(
    ['hint' => '空字节截断：file.php%00.jpg', 'full' => '上传 file.php%00.jpg（PHP 5.x）'],
    [
    '原理' => '文件上传第 8 关：利用空字节截断，file.php%00.jpg（PHP 5.x 漏洞演示）。<br>文件上传漏洞核心：服务器未正确验证上传文件类型，导致攻击者上传可执行文件（如 .php）获取 webshell。',
...
if ($allow) move_uploaded_file($_FILES["file"]["tmp_name"], UPLOAD_DIR.$name);</pre><span class="danger">↑ 验证不充分</span>',
    '攻击演示' => '1. 尝试上传 .php 文件<br>2. 根据验证逻辑构造绕过 payload<br>3. 上传成功后访问 /uploads/shell.php → 通关<br>payload：上传 file.php%00.jpg（PHP 5.x）',
    'payload详解' => '<code class="payload">上传 file.php%00.jpg（PHP 5.x）</code><br>提示：空字节截断：file.php%00.jpg',
    '工具实操' => 'curl -F file=@shell.php http://靶场/stage8.php<br>Burp Suite 抓包修改 filename / Content-Type<br>上传后访问 /uploads/验证执行',
...
$newname = bin2hex(random_bytes(16)) . "." . $ext;</pre>',
    '知识卡片' => '📌 文件上传漏洞可导致 RCE（远程代码执行）<br>OWASP A04:2021-Insecure Design<br>关键防御：白名单 > 黑名单，内容检测 > 扩展名检测<br>本关演示：空字节绕过'
],
    __FILE__
);
