...
if ($passed) pass_stage('upload', 14, 'upload ' . ($_FILES['file']['name'] ?? ''));

upload_head(14, '路径穿越', '文件名含 ../ 写到其他目录。');
?>
...
<?php
upload_tail(
    ['hint' => '文件名含 ../../写到上级目录', 'full' => 'filename=../../test.php'],
    [
    '原理' => '文件上传第 14 关：文件名含 ../ 写到其他目录。<br>文件上传漏洞核心：服务器未正确验证上传文件类型，导致攻击者上传可执行文件（如 .php）获取 webshell。',
...
if ($allow) move_uploaded_file($_FILES["file"]["tmp_name"], UPLOAD_DIR.$name);</pre><span class="danger">↑ 验证不充分</span>',
    '攻击演示' => '1. 尝试上传 .php 文件<br>2. 根据验证逻辑构造绕过 payload<br>3. 上传成功后访问 /uploads/shell.php → 通关<br>payload：filename=../../test.php',
    'payload详解' => '<code class="payload">filename=../../test.php</code><br>提示：文件名含 ../../写到上级目录',
    '工具实操' => 'curl -F file=@shell.php http://靶场/stage14.php<br>Burp Suite 抓包修改 filename / Content-Type<br>上传后访问 /uploads/验证执行',
...
$newname = bin2hex(random_bytes(16)) . "." . $ext;</pre>',
    '知识卡片' => '📌 文件上传漏洞可导致 RCE（远程代码执行）<br>OWASP A04:2021-Insecure Design<br>关键防御：白名单 > 黑名单，内容检测 > 扩展名检测<br>本关演示：路径穿越'
],
    __FILE__
);
