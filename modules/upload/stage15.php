...
if ($passed) pass_stage('upload', 15, 'upload ' . ($_FILES['file']['name'] ?? ''));

upload_head(15, 'ZIP滑块', '嵌套 ZIP 利用解压覆盖。');
?>
...
if ($allow) move_uploaded_file($_FILES["file"]["tmp_name"], UPLOAD_DIR.$name);</pre><span class="danger">↑ 验证不充分</span>',
    '攻击演示' => '1. 尝试上传 .php 文件<br>2. 根据验证逻辑构造绕过 payload<br>3. 上传成功后访问 /uploads/shell.php → 通关<br>payload：使用 ZIP 滑块工具构造',
    'payload详解' => '<code class="payload">使用 ZIP 滑块工具构造</code><br>提示：构造嵌套 ZIP 文件',
    '工具实操' => 'curl -F file=@shell.php http://靶场/stage15.php<br>Burp Suite 抓包修改 filename / Content-Type<br>上传后访问 /uploads/验证执行',
...
$newname = bin2hex(random_bytes(16)) . "." . $ext;</pre>',
    '知识卡片' => '📌 文件上传漏洞可导致 RCE（远程代码执行）<br>OWASP A04:2021-Insecure Design<br>关键防御：白名单 > 黑名单，内容检测 > 扩展名检测<br>本关演示：ZIP滑块'
],
    __FILE__
);
