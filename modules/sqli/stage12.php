...
sqli_head(12, '布尔盲注-括号', '括号+单引号布尔盲注。');
sqli_form('id', $id);
sqli_error($err); sqli_pass($passed);
sqli_result($rows);
sqli_tail(
    ['hint' => '闭合 \') 后布尔判断', 'full' => '1\') AND (\'1\'=\'1'],
    [
        '原理' => '需闭合括号和引号。',
        ...
        '工具实操' => 'sqlmap -u "stage12.php?id=1" --batch --dbs<br>Burp Repeater 调试 payload。',
        '通关检测' => '返回行数 &gt; 1 或结果含 flag 即通关。',
        '防御修复' => 'medium: addslashes（可被宽字节绕过）；high: intval 强转；impossible: PDO 预处理参数化查询。',
        '知识卡片' => '📌 布尔盲注-括号。关联：闭合方式 → 报错/盲注 → WAF 绕过。真实案例：SQL 注入长期位居 OWASP Top 10。',
    ],
    __FILE__
);
