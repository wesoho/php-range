...
sqli_head(20, 'update 注入', 'UPDATE SET 注入，修改数据或注入子查询。');
sqli_form('id', $id);
sqli_error($err); sqli_pass($passed);
sqli_result($rows);
sqli_tail(
    ['hint' => '在 SET 或 WHERE 注入', 'full' => '1 OR password LIKE \'flag%'],
    [
        '原理' => 'UPDATE 注入：WHERE 条件注入，修改匹配行。',
        '漏洞代码' => '<pre>$sql = "SELECT ... WHERE id=$id";</pre><span class="danger">↑ 参数直接拼接</span>',
        '攻击演示' => '1 OR password LIKE \'flag%\' 使 UPDATE 影响所有行。',
        'payload详解' => 'payload：<code class="payload">1 OR password LIKE \'flag%</code><br>根据闭合方式构造，闭合引号/括号后注入条件或 UNION。',
        '工具实操' => 'sqlmap -u "stage20.php?id=1" --batch --dbs<br>Burp Repeater 调试 payload。',
        '通关检测' => '返回行数 &gt; 1 或结果含 flag 即通关。',
        '防御修复' => 'medium: addslashes（可被宽字节绕过）；high: intval 强转；impossible: PDO 预处理参数化查询。',
        '知识卡片' => '📌 update 注入。关联：闭合方式 → 报错/盲注 → WAF 绕过。真实案例：SQL 注入长期位居 OWASP Top 10。',
    ],
    __FILE__
);
