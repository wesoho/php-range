...
if ($passed) pass_stage('cmdi', 8, 'ip=' . $ip);

cmdi_head(8, '内联注释绕过', '过滤 ; | & ` 但可用 $() 绕过。');
?>
...
<?php
cmdi_tail(
    ['hint' => '过滤了 ; | & `，用 $() 绕过', 'full' => '127.0.0.1;$(id)'],
    [
    '原理' => '过滤 ; | & ` 后，bash 下 \$() 命令替换仍然可用（替换结果拼进 ping 参数，错误信息回显 uid=）；<b>Windows cmd.exe 不支持 \$()，此过滤在 Windows 下有效</b>。',
    '漏洞代码' => '<pre>$ip = $_GET["ip"];
        $ip = str_replace(array(\';\',\'|\',\'&\',\'`\'), \'\', $ip);
$cmd = "ping -c 1 " . $ip;
exec($cmd, $output);</pre><span class="danger">↑ 输入直接拼入系统命令</span>',
    ...
    'payload详解' => '<code class="payload">127.0.0.1;$(id)</code><br>提示：过滤了 ; | & `，用 $() 绕过<br>命令注入核心：用分隔符（; | & 等）结束原命令，开始新命令。',
    '工具实操' => 'curl "http://靶场/stage8.php?ip=127.0.0.1;id"<br>Burp Suite Repeater 调试 payload<br>注意：不同操作系统命令分隔符不同',
...
if (!filter_var($ip, FILTER_VALIDATE_IP)) die("无效IP");</pre>',
    '知识卡片' => '📌 命令注入 = RCE（远程代码执行）<br>OWASP A03:2021-Injection<br>分隔符：; | & || && $() ` %0a<br>本关演示：内联注释绕过'
],
    __FILE__
);
