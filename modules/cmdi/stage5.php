...
if ($passed) pass_stage('cmdi', 5, 'ip=' . $ip);

cmdi_head(5, '过滤多种符号', '过滤 ; | & 但可用换行 %0a 绕过。');
?>
...
<?php
cmdi_tail(
    ['hint' => '过滤了 ; | &，用换行 %0a 绕过', 'full' => '127.0.0.1%0aid'],
    [
    '原理' => '过滤分号/管道/与符号后，bash 可用换行 %0a 绕过；<b>Windows cmd.exe 没有等价分隔符，此过滤在 Windows 下确实有效</b>——这也说明黑名单过滤从来不可靠，Linux 下照样被绕过。',
    '漏洞代码' => '<pre>$ip = $_GET["ip"];
        $ip = str_replace(array(\';\',\'|\',\'&\'), \'\', $ip);
$cmd = "ping -c 1 " . $ip;
exec($cmd, $output);</pre><span class="danger">↑ 输入直接拼入系统命令</span>',
    '攻击演示' => '1. 输入 127.0.0.1 正常 ping<br>2. 输入 127.0.0.1%0aid<br>3. 观察输出中是否包含注入命令结果 → 通关',
    'payload详解' => '<code class="payload">127.0.0.1%0aid</code><br>提示：过滤了 ; | &，用换行 %0a 绕过<br>命令注入核心：用分隔符（; | & 等）结束原命令，开始新命令。',
    '工具实操' => 'curl "http://靶场/stage5.php?ip=127.0.0.1;id"<br>Burp Suite Repeater 调试 payload<br>注意：不同操作系统命令分隔符不同',
...
if (!filter_var($ip, FILTER_VALIDATE_IP)) die("无效IP");</pre>',
    '知识卡片' => '📌 命令注入 = RCE（远程代码执行）<br>OWASP A03:2021-Injection<br>分隔符：; | & || && $() ` %0a<br>本关演示：过滤多种符号'
],
    __FILE__
);
