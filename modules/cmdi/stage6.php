...
if ($passed) pass_stage('cmdi', 6, 'ip=' . $ip);

cmdi_head(6, '过滤空格', '过滤空格但可用 $IFS 或 {cat,/etc/passwd} 绕过。');
?>
...
exec($cmd, $output);</pre><span class="danger">↑ 输入直接拼入系统命令</span>',
    '攻击演示' => '1. 输入 127.0.0.1 正常 ping<br>2. 输入 127.0.0.1;cat${IFS}/etc/passwd<br>3. 观察输出中是否包含注入命令结果 → 通关',
    'payload详解' => '<code class="payload">127.0.0.1;cat${IFS}/etc/passwd</code><br>提示：过滤了空格，用 $IFS 或大括号绕过<br>命令注入核心：用分隔符（; | & 等）结束原命令，开始新命令。',
    '工具实操' => 'curl "http://靶场/stage6.php?ip=127.0.0.1;id"<br>Burp Suite Repeater 调试 payload<br>注意：不同操作系统命令分隔符不同',
...
if (!filter_var($ip, FILTER_VALIDATE_IP)) die("无效IP");</pre>',
    '知识卡片' => '📌 命令注入 = RCE（远程代码执行）<br>OWASP A03:2021-Injection<br>分隔符：; | & || && $() ` %0a<br>本关演示：过滤空格'
],
    __FILE__
);
