...
if ($passed) pass_stage('cmdi', 1, 'ip=' . $ip);

cmdi_head(1, '无过滤命令注入', 'ping 命令直接拼接用户输入，无任何过滤。');
?>
...
<?php
cmdi_tail(
    ['hint' => '用分号或管道符拼接命令', 'full' => '127.0.0.1;id　或　127.0.0.1&whoami'],
    [
    '原理' => '命令注入第 1 关：ping 命令直接拼接用户输入，无任何过滤。<br>OS 命令注入：用户输入被拼接到系统命令中，攻击者通过特殊字符（; | & $() ` 等）注入额外命令。',
    '漏洞代码' => '<pre>$ip = $_GET["ip"];

$cmd = "ping -c 1 " . $ip;
exec($cmd, $output);</pre><span class="danger">↑ 输入直接拼入系统命令</span>',
    '攻击演示' => '1. 输入 127.0.0.1 正常 ping<br>2. 输入 127.0.0.1;id 或 127.0.0.1|id<br>3. 观察输出中是否包含注入命令结果 → 通关',
    'payload详解' => '<code class="payload">127.0.0.1;id 或 127.0.0.1|id</code><br>提示：用分号或管道符拼接命令<br>命令注入核心：用分隔符（; | & 等）结束原命令，开始新命令。',
    '工具实操' => 'curl "http://靶场/stage1.php?ip=127.0.0.1;id"<br>Burp Suite Repeater 调试 payload<br>注意：不同操作系统命令分隔符不同',
...
if (!filter_var($ip, FILTER_VALIDATE_IP)) die("无效IP");</pre>',
    '知识卡片' => '📌 命令注入 = RCE（远程代码执行）<br>OWASP A03:2021-Injection<br>分隔符：; | & || && $() ` %0a<br>本关演示：无过滤命令注入'
],
    __FILE__
);
