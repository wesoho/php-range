...
if ($passed) pass_stage('xss', 5, 'keyword=' . $input);

xss_head(5, '反射型-过滤大小写', '用正则 /i 过滤 script，但可用其他标签绕过。');
?>
...
<?php
xss_tail(
    ['hint' => '正则 /i 过滤了 script，但可以用 img onerror 绕过', 'full' => '<img src=x onerror=alert(1)>'],
    [
    '原理' => '反射型 XSS 第 5 关：用正则 /i 过滤 script，但可用其他标签绕过。<br>用户输入通过 URL 参数传入，服务端处理后直接输出到 HTML。',
...
echo $input;</pre><span class="danger">↑ 输出未转义</span>',
    ...
    'payload详解' => '<code class="payload"><img src=x onerror=alert(1)></code><br>提示：正则 /i 过滤了 script，但可以用 img onerror 绕过',
    ...
    '知识卡片' => '📌 XSS 三类型：反射型/存储型/DOM型<br>OWASP A03:2021-Injection<br>本关演示：反射型-过滤大小写'
],
    __FILE__
);
