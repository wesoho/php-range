...
if ($passed) pass_stage('xss', 6, 'keyword=' . $input);

xss_head(6, '反射型-双写绕过', 'str_replace 单次替换，可用双写绕过。');
?>
...
<?php
xss_tail(
    ['hint' => 'str_replace 单次替换，双写绕过', 'full' => '<scrscriptipt>alert(1)</scrscriptipt>'],
    [
    '原理' => '反射型 XSS 第 6 关：str_replace 单次替换，可用双写绕过。<br>用户输入通过 URL 参数传入，服务端处理后直接输出到 HTML。',
    '漏洞代码' => '<pre>$input = $_GET["keyword"];
        $input = str_replace(\'script\', \'\', $input);
echo $input;</pre><span class="danger">↑ 输出未转义</span>',
    '攻击演示' => '1. 正常输入 test<br>2. 注入 XSS payload → 弹窗 → 通关<br>3. payload：<scrscriptipt>alert(1)</scrscriptipt>',
    'payload详解' => '<code class="payload"><scrscriptipt>alert(1)</scrscriptipt></code><br>提示：str_replace 单次替换，双写绕过',
    ...
    '知识卡片' => '📌 XSS 三类型：反射型/存储型/DOM型<br>OWASP A03:2021-Injection<br>本关演示：反射型-双写绕过'
],
    __FILE__
);
