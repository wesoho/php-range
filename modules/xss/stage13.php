...
if ($content !== '') {
    ...
    if ($level === 'impossible' || $level === 'high') {
        ...
        $to_store = preg_replace('/on(\w+)\s*=/i', '', $to_store);
    } else {
        $to_store = preg_replace('/on(\w+)\s*=/i', '', $to_store);
    }
...
if ($passed) pass_stage('xss', 13, 'content=' . $content);

xss_head(13, '存储型-过滤on事件', '留言板过滤 on 事件，但可用 script 或编码绕过。');
?>
...
<?php
xss_tail(
    ['hint' => '过滤了 on 事件，用 script 绕过', 'full' => '<script>alert(1)</script>'],
    [
    '原理' => '存储型 XSS 第 13 关：留言板过滤 on 事件，但可用 script 或编码绕过。<br>用户提交的留言存入数据库，后续访问时输出到页面。攻击持久存在。',
...
echo $row["content"];</pre><span class="danger">↑ 存入和输出均未转义</span>',
    ...
    '知识卡片' => '📌 存储型 XSS 危害最大<br>Samy 蠕虫 2005 MySpace 24h 感染 100 万用户<br>本关演示：存储型-过滤on事件'
],
    __FILE__
);
