...
if ($passed) pass_stage('xss', 10, 'keyword=' . $input);

xss_head(10, '反射型-HTML实体编码', '过滤了尖括号，但可用编码绕过。');
?>
...
<?php
xss_tail(
    ['hint' => '过滤了尖括号，用 javascript: 伪协议绕过', 'full' => 'javascript:alert(1) 写在 href 或事件中'],
    [
    '原理' => '反射型 XSS 第 10 关：过滤了尖括号，但可用编码绕过。<br>用户输入通过 URL 参数传入，服务端处理后直接输出到 HTML。',
    '漏洞代码' => '<pre>$input = $_GET["keyword"];
        $input = str_replace(array(\'<\',\'>\'), \'\', $input);
echo $input;</pre><span class="danger">↑ 输出未转义</span>',
    '攻击演示' => '1. 正常输入 test<br>2. 注入 XSS payload → 弹窗 → 通关<br>3. payload：javascript:alert(1) 写在 href 或事件中',
    'payload详解' => '<code class="payload">javascript:alert(1) 写在 href 或事件中</code><br>提示：过滤了尖括号，用 javascript: 伪协议绕过',
    ...
    '知识卡片' => '📌 XSS 三类型：反射型/存储型/DOM型<br>OWASP A03:2021-Injection<br>本关演示：反射型-HTML实体编码'
],
    __FILE__
);
