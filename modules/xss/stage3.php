...
if ($passed) pass_stage('xss', 3, 'keyword=' . $input);

xss_head(3, '反射型-单引号属性', '输入嵌入在 input value 单引号属性中。');
?>
...
<?php
xss_tail(
    ['hint' => '输入在单引号属性中，需要先闭合引号', 'full' => '\'><script>alert(1)</script>'],
    [
    '原理' => '反射型 XSS 第 3 关：输入嵌入在 input value 单引号属性中。<br>用户输入通过 URL 参数传入，服务端处理后直接输出到 HTML。',
...
echo $input;</pre><span class="danger">↑ 输出未转义</span>',
    ...
    'payload详解' => '<code class="payload">\'><script>alert(1)</script></code><br>提示：输入在单引号属性中，需要先闭合引号',
    ...
    '知识卡片' => '📌 XSS 三类型：反射型/存储型/DOM型<br>OWASP A03:2021-Injection<br>本关演示：反射型-单引号属性'
],
    __FILE__
);
